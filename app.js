...
let pageBckground = () => {
  ...
  if (userBckground == null || userBckground == "") {
    localStorage.setItem("chat_project_background", "#085f63");
  }
...
btnSetDates.addEventListener("click", (event) => {
  ...
  if (chatShowStart == "" || chatShowEnd == "") {
    ...
    ulChatList.clear();
    chatroom.getChatsDate(chatShowStart, chatShowEnd, (data) => {
      ulChatList.templateLI(data);
    });
  }
});
...
function checkUsername() {
  ...
  document.getElementById(
    "inputUsername"
  ).placeholder = `Type new username: ${getUsername}`;
...
rooms.addEventListener("click", (event) => {
  ...
  if (event.target.className.includes("roomBtns")) {
    //this if so if you click on ul or div tag don't do nothing
...
btnUpdate.addEventListener("click", (event) => {
  ...
  if (
    username.value != "" ||
    username.value != localStorage.getItem("chat_project_username")
  ) {
    localStorage.setItem("chat_project_username", username.value);
    chatroom.updateUsername(username.value);
    document.getElementById(
      "inputUsername"
    ).placeholder = `Type new username: ${username.value}`;
    ...
    alert(
      "You can't put EMPTY username or You already have set the same username!"
    );
  }
  checkUsername();
});
